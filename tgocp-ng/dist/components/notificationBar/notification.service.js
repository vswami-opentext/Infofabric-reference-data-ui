var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from '@angular/core';
import { Subject } from 'rxjs';
import { NotificationProperties } from "./notification.properties";
var NotificationService = /** @class */ (function () {
    function NotificationService() {
        this.notificationSubject = new Subject();
        this.notificationState = this.notificationSubject.asObservable();
        this.properties = new NotificationProperties();
    }
    NotificationService.prototype.show = function (properties) {
        var _this = this;
        this.properties = properties;
        properties.show = true;
        this.notificationSubject.next(this.properties);
        setTimeout(function () {
            _this.properties.show = false;
            _this.notificationSubject.next(_this.properties);
        }, this.properties.duration);
    };
    NotificationService.prototype.hide = function () {
        this.properties.show = false;
        this.notificationSubject.next(this.properties);
    };
    NotificationService = __decorate([
        Injectable(),
        __metadata("design:paramtypes", [])
    ], NotificationService);
    return NotificationService;
}());
export { NotificationService };
//# sourceMappingURL=notification.service.js.map